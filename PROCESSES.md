...
| Live PM view | Show the workspace as a board: lifecycle columns, age in state, milestones | [`board`](skills/board/SKILL.md); any state, read-only | local read-only web view | Never writes workspace state and never invokes operations. A board edit path would reintroduce ungated state editing. |
| Session context | Give an agent compact current state plus the non-negotiable rules | [`prime`](skills/prime/SKILL.md); any state, read-only | ~30-line digest | Summary, not authority: METHOD.md, the schemas, and the owning skill win on conflict. |
