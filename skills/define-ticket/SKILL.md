...
5. Record unresolved human choices under open decisions. Never invent intent or trade-offs.
   When a human resolves a choice and asks to retain it durably, use `a-team decision create --from <draft.md> --approve` instead of editing `.a-team/decisions/`.
6. Create the item with `a-team ticket new`, write the investigated definition to a temporary Markdown file, then apply it with `a-team ticket define <ticket-id> --from <file>`. Keep incomplete work in backlog.
7. Run `a-team ticket validate <ticket-id>` and `a-team validate`.
