---
name: define-ticket
description: Turn a raw request or backlog item into a concise, evidence-grounded Kotta ticket. Use when a user asks to define, refine, clarify, create, update, or make a Kotta ticket ready for execution.
---

# Define a ticket

Investigate before asking the human. Use the `kotta` CLI for every ticket creation or lifecycle mutation; do not move or rewrite canonical ticket files directly.
...
6. Create the item with `kotta ticket new`, write the investigated definition to a temporary Markdown file, then apply it with `kotta ticket define <ticket-id> --from <file>`. Keep incomplete work in backlog.
7. Run `kotta ticket validate <ticket-id>` and `kotta validate`.
8. Only after the definition is complete and required human approval is present, run `kotta ticket ready <ticket-id>`.
