...
function validateContract(path: string, expectedState?: string, requireDefinition = false): ValidationReport {
  const errors: ValidationIssue[] = [];
  if (!existsSync(path)) return { valid: false, errors: [{ code: "NOT_FOUND", message: `Contract not found: ${path}`, path }] };
  let entity;
  ...
  if (expectedState && state !== expectedState) errors.push({ code: "WRONG_STATE", message: `Expected contract state '${expectedState}', found '${state}'.`, path });
  ...
  if (state === "done" && !["completed", "cancelled", "duplicate", "obsolete"].includes(String(entity.data.resolution))) errors.push({ code: "MISSING_RESOLUTION", message: "Done contract requires a final resolution.", path });
...
export function validateContractFile(path: string, expectedState?: string): ValidationReport {
  return validateContract(path, expectedState);
}

export function validateContractDefinitionFile(path: string): ValidationReport {
  return validateContract(path, "backlog", true);
}
