...
import { validateTicketFile } from "../../src/core/validation.js";
...
const codes = (path: string) => validateTicketFile(path).errors.map((error) => error.code);

describe("deviation reconciliation (F-019)", () => {
  test("'None.' plus a deviation in the narrative fails validation", () => {
    const path = ticket({ deviations: "None.", verification: "flow.md inspected.\nDEVIACIOK: a HTTP endpoint nem keszult el." });
    expect(codes(path)).toContain("DEVIATION_MISMATCH");
    expect(validateTicketFile(path).errors.find((error) => error.code === "DEVIATION_MISMATCH")?.message).toContain("DEVIACIOK");
  });

  test("'Not declared.' plus a deviation in the narrative fails validation", () => {
    const path = ticket({ deviations: "Not declared.", verification: "flow.md inspected.\nOne deviation: the diagram was skipped." });
    expect(codes(path)).toContain("DEVIATION_MISMATCH");
  });
  ...
  test("'None.' with no deviation marker in the narrative passes", () => {
    const path = ticket({ deviations: "None.", verification: "flow.md inspected; every acceptance condition met." });
    expect(codes(path)).not.toContain("DEVIATION_MISMATCH");
  });

  test("a narrative that denies deviations agrees with the field and passes", () => {
    const path = ticket({ deviations: "None.", verification: "flow.md inspected. No deviations, findings, or known concerns.\nErdemi deviacio nincs." });
    expect(codes(path)).not.toContain("DEVIATION_MISMATCH");
  });

  test("the rule does not run before done", () => {
    const path = ticket({ state: "review", deviations: "None.", verification: "flow.md inspected.\nDEVIACIOK: a HTTP endpoint nem keszult el." });
    expect(codes(path)).not.toContain("DEVIATION_MISMATCH");
  });
});
